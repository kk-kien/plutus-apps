...
module Legacy.Plutus.V1.Ledger.Tx(
    -- * Transactions
    Tx(..),
    inputs,
    collateralInputs,
...
import Plutus.V1.Ledger.Contexts (TxOut (txOutValue))
import Plutus.V1.Ledger.Tx (RedeemerPtr, Redeemers, ScriptTag, TxId, TxIn (txInRef), TxInType, TxOutRef, txOutDatum)
