...
txnValidFrom =
    let five = Value.singleton Value.adaSymbol Value.adaToken 5
...
invalidTrace = property $ do
    (Mockchain m _ _, txn) <- forAll genChainTxn
    let invalidTxn = txn { txMint = Value.singleton Value.adaSymbol Value.adaToken 1 }
...
pubKeyTransactions2 = do
    let payment1 = initialBalance P.- Value.singleton Value.adaSymbol Value.adaToken 10
        payment2 = initialBalance P.+ Value.singleton Value.adaSymbol Value.adaToken 10
    ...
    _ <- Trace.nextSlot
    Trace.liftWallet wallet1 $ payToPaymentPublicKeyHash_ W.always (Value.singleton Value.adaSymbol Value.adaToken 20) pubKey2
    void Trace.nextSlot
...
initialBalance = Value.singleton Value.adaSymbol Value.adaToken 100
