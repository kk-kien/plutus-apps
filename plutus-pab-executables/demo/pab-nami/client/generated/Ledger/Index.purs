...
data ScriptValidationEvent
  ...
  | ResultOnlyEvent { sveResult :: Either ScriptError (Tuple RawJson (Array String)) }
...
instance EncodeJson ScriptValidationEvent where
  encodeJson = defer \_ -> case _ of
    ...
    ResultOnlyEvent { sveResult } -> encodeJson
      { tag: "ResultOnlyEvent"
      , sveResult: flip E.encode sveResult (E.either E.value (E.tuple (E.value >/\< E.value)))
      }
...
instance DecodeJson ScriptValidationEvent where
  decodeJson = defer \_ -> D.decode
    ...
    $ Map.fromFoldable
        ...
        , "ResultOnlyEvent" /\ (ResultOnlyEvent <$> D.object "ResultOnlyEvent" { sveResult: (D.either D.value (D.tuple (D.value </\> D.value))) :: _ (Either ScriptError (Tuple RawJson (Array String))) })
        ]
...
_ResultOnlyEvent :: Prism' ScriptValidationEvent { sveResult :: Either ScriptError (Tuple RawJson (Array String)) }
_ResultOnlyEvent = prism' ResultOnlyEvent case _ of
  (ResultOnlyEvent a) -> Just a
  _ -> Nothing
